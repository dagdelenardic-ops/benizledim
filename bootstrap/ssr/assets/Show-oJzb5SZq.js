import { mergeProps, useSSRContext, ref, computed, unref, withCtx, createTextVNode, toDisplayString, onMounted, onUnmounted, onBeforeUnmount, openBlock, createBlock, createVNode, Fragment, renderList, createCommentVNode } from "vue";
...
import { a as _export_sfc, A as AppLayout, _ as _sfc_main$i } from "./AppLayout-DRxfLCYb.js";
import { _ as _sfc_main$h, b as buildResponsiveImage } from "./PostCard-C09KKzTl.js";
...
const _sfc_main$f = {
  ...
  setup(__props, { emit: __emit }) {
    ...
    function slugifyName(name) {
      return (name || "").toLocaleLowerCase("tr-TR").normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/[^a-z0-9\s-]/g, "").trim().replace(/\s+/g, "-").replace(/-+/g, "-");
    }
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "relative" }, _attrs))}><textarea${ssrRenderAttr("rows", __props.rows)}${ssrRenderAttr("placeholder", __props.placeholder)}${ssrRenderAttr("maxlength", __props.maxLength)}${ssrIncludeBooleanAttr(__props.disabled) ? " disabled" : ""} class="w-full resize-none rounded-lg border border-gray-200 px-4 py-3 transition-all focus:outline-none focus:border-red-500 focus:ring-2 focus:ring-red-500/20 disabled:cursor-not-allowed disabled:bg-gray-50">${ssrInterpolate(value.value)}</textarea>`);
      if (showSuggestions.value) {
        _push(`<div class="absolute left-0 right-0 top-full z-20 mt-2 overflow-hidden rounded-lg border border-gray-200 bg-white shadow-lg"><!--[-->`);
        ssrRenderList(suggestions.value, (author, index) => {
          _push(`<button type="button" class="${ssrRenderClass([index === activeIndex.value ? "bg-red-50" : "hover:bg-gray-50", "flex w-full items-center gap-3 px-3 py-2 text-left transition-colors"])}">`);
          if (author.avatar) {
            _push(`<img${ssrRenderAttr("src", author.avatar)}${ssrRenderAttr("alt", author.name)} class="h-8 w-8 rounded-full object-cover">`);
          } else {
            _push(`<div class="flex h-8 w-8 items-center justify-center rounded-full bg-red-100 text-xs font-black text-red-700">${ssrInterpolate(author.name.charAt(0))}</div>`);
          }
          _push(`<div class="min-w-0"><p class="truncate text-sm font-semibold text-gray-900">${ssrInterpolate(author.name)}</p><p class="truncate text-xs text-gray-500">@${ssrInterpolate(author.username || slugifyName(author.name))}</p></div></button>`);
        });
        _push(`<!--]--></div>`);
      } else {
        _push(`<!---->`);
...
_sfc_main$f.setup = (props, ctx) => {
  const ssrContext = useSSRContext();
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("resources/js/Components/UI/MentionTextarea.vue");
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (!unref(authUser)) {
        ...
        _push(ssrRenderComponent(_sfc_main$f, {
          ...
          class: { "border-red-500 focus:border-red-500": isOverLimit.value }
        }, null, _parent));
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(ssrRenderComponent(_sfc_main$f, {
        ...
        disabled: unref(form).processing,
        class: ["text-sm", { "border-red-400": isOverLimit.value }]
      }, null, _parent));
      _push(`<div class="${ssrRenderClass([{
        "text-gray-400": !isNearLimit.value && !isOverLimit.value,
        "text-orange-500": isNearLimit.value && !isOverLimit.value,
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (__props.isRevisit && __props.originalPost) {
        ...
        _push(` yazısının ${ssrInterpolate(timeDifference.value)} sonra tekrar değerlendirmesidir. </p><p class="text-xs text-amber-500 mt-1"> Orijinal: ${ssrInterpolate(unref(formatDate)(__props.originalPost.published_at))}</p></div></div>`);
      } else {
        _push(`<!---->`);
      }
...
const _sfc_main$3 = {
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "bi-article-body" }, _attrs))} data-v-d4039154>${sanitized.value ?? ""}</div>`);
    };
  }
};
...
const ArticleBody = /* @__PURE__ */ _export_sfc(_sfc_main$3, [["__scopeId", "data-v-d4039154"]]);
const _sfc_main$2 = {
  __name: "WatchlistButton",
  __ssrInlineRender: true,
  props: {
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<button${ssrRenderAttrs(mergeProps({
        ...
        title: watchlisted.value ? "Watchlistten çıkar" : "Watchliste ekle"
      }, _attrs))}><svg class="h-4 w-4"${ssrRenderAttr("fill", watchlisted.value ? "currentColor" : "none")} stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 5a2 2 0 012-2h10a2 2 0 012 2v16l-7-4-7 4V5z"></path></svg></button>`);
    };
  }
};
...
_sfc_main$2.setup = (props, ctx) => {
  const ssrContext = useSSRContext();
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("resources/js/Components/Social/WatchlistButton.vue");
  return _sfc_setup$2 ? _sfc_setup$2(props, ctx) : void 0;
};
const gururSonmez = "/build/assets/gurur-sonmez-Cie7FSYk.jpg";
const irisEryilmaz = "/build/assets/iris-eryilmaz-Hd1okyfx.jpg";
const muhammedMuglu = "/build/assets/muhammed-muglu-C_-bp9-n.jpg";
const suEvci = "/build/assets/su-evci-yqXgXV2t.jpg";
const alphanKarabat = "/build/assets/alphan-karabat-NgQU-WRf.jpg";
const humeyraFidan = "/build/assets/humeyra-fidan-CtZklFs4.jpg";
const gokceSerim = "/build/assets/gokce-serim-DpbPnV8r.jpg";
const melikeZeynepKorkmaz = "/build/assets/melike-zeynep-korkmaz-BBtGbnQG.png";
const huseyinEmreSepetci = "/build/assets/huseyin-emre-sepetci-DZ3YGYI9.jpg";
const normalize = (value = "") => value.toLocaleLowerCase("tr-TR").replace(/i̇/g, "i").normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/ı/g, "i").replace(/[^a-z0-9]+/g, " ").trim();
const AVATARS = {
...
const getAuthorAvatar = (name = "") => AVATARS[normalize(name)] || null;
const _sfc_main$1 = {
  __name: "PullQuote",
  __ssrInlineRender: true,
  props: {
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      }, _attrs))} data-v-d2789d2e><svg class="pull-quote__mark" viewBox="0 0 100 100" aria-hidden="true" focusable="false" data-v-d2789d2e><path d="M42 20C25 27 14 42 14 62c0 11 7 18 17 18 9 0 16-7 16-16 0-8-6-15-14-16-1 0-2 0-3 .3 2-8 9-15 18-19zM88 20C71 27 60 42 60 62c0 11 7 18 17 18 9 0 16-7 16-16 0-8-6-15-14-16-1 0-2 0-3 .3 2-8 9-15 18-19z" data-v-d2789d2e></path></svg><p class="pull-quote__text" data-v-d2789d2e>${ssrInterpolate(__props.text)}</p>`);
      if (__props.authorName) {
        _push(`<div class="pull-quote__by" data-v-d2789d2e><span class="pull-quote__rule" aria-hidden="true" data-v-d2789d2e></span>`);
        if (resolvedAvatar.value) {
          _push(`<img${ssrRenderAttr("src", resolvedAvatar.value)}${ssrRenderAttr("alt", __props.authorName)} class="pull-quote__avatar" width="44" height="44" loading="lazy" decoding="async" data-v-d2789d2e>`);
        } else {
          _push(`<span class="pull-quote__avatar pull-quote__avatar--fallback" data-v-d2789d2e>${ssrInterpolate(initial.value)}</span>`);
        }
        _push(`<span class="pull-quote__meta" data-v-d2789d2e><span class="pull-quote__name" data-v-d2789d2e>${ssrInterpolate(__props.authorName)}</span><span class="pull-quote__role" data-v-d2789d2e>${ssrInterpolate(__props.authorRole)}</span></span></div>`);
      } else {
        _push(`<!---->`);
      }
      _push(`</aside>`);
...
_sfc_main$1.setup = (props, ctx) => {
  const ssrContext = useSSRContext();
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("resources/js/Components/Post/PullQuote.vue");
  return _sfc_setup$1 ? _sfc_setup$1(props, ctx) : void 0;
};
const PullQuote = /* @__PURE__ */ _export_sfc(_sfc_main$1, [["__scopeId", "data-v-d2789d2e"]]);
const _sfc_main = {
  __name: "Show",
  __ssrInlineRender: true,
  props: {
  ...
  setup(__props) {
    ...
    const stripWixArtifacts = (html = "") => {
      let cleaned = html.replace(/<footer\b[\s\S]*?<\/footer>\s*$/gi, "");
    ...
    const analyzeContent = (rawHtml = "") => {
      ...
      const wixOriginal = (storagePath) => {
        ...
        return /(~mv2|^[0-9a-f]{4,8}_[0-9a-f]{16,})/i.test(file) ? WIX_CDN + encodeURIComponent(file) : "";
      };
      let htmlWithImages = processedHtml.replace(/<img\b[^>]*>/gi, (tag) => {
        ...
        const fallbackAttribute = fallback ? ` data-fallback-src="${fallback}"` : "";
        const injected = ` srcset="${responsive.srcset}" sizes="${responsive.sizes}"` + fallbackAttribute + (/\sloading\s*=/i.test(rebuilt) ? "" : ' loading="lazy"') + (/\sdecoding\s*=/i.test(rebuilt) ? "" : ' decoding="async"');
    ...
    const showExcerpt = computed(() => {
      ...
      const normalize2 = (value = "") => value.toLocaleLowerCase("tr-TR").replace(/[^\p{L}\p{N}\s]/gu, "").replace(/\s+/g, " ").trim();
      const probe = normalize2(excerpt).slice(0, 60);
      if (!probe) return false;
      return !normalize2(stripHtml(processedContent.value)).startsWith(probe);
    });
      ...
      _push(ssrRenderComponent(AppLayout, mergeProps({
        ...
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            ...
            ssrRenderList(__props.post.categories.slice(0, 3), (category) => {
            ...
            _push2(`<!--]-->`);
            if (__props.post.categories.length > 3) {
              _push2(`<span class="px-3 py-1 bg-gray-100 text-gray-500 text-sm font-medium rounded-full"${ssrRenderAttr("title", __props.post.categories.slice(3).map((c) => c.name).join(", "))}${_scopeId}> +${ssrInterpolate(__props.post.categories.length - 3)}</span>`);
            } else {
              _push2(`<!---->`);
            }
                ...
                default: withCtx((_2, _push3, _parent3, _scopeId2) => {
                  if (_push3) {
                    if (authorAvatar.value) {
                      _push3(`<img${ssrRenderAttr("src", authorAvatar.value)}${ssrRenderAttr("alt", __props.post.user.name)} class="w-10 h-10 rounded-full object-cover" loading="lazy" decoding="async"${_scopeId2}>`);
                    } else {
            ...
            if (showExcerpt.value) {
              _push2(ssrRenderComponent(PullQuote, {
                text: __props.post.excerpt,
                "author-name": __props.post.user?.name || ""
              }, null, _parent2, _scopeId));
            } else {
              _push2(`<!---->`);
            }
            ...
            _push2(`<div class="mx-auto mt-8 flex max-w-[750px] flex-col gap-4 border-t border-gray-200 pt-8 sm:flex-row sm:items-center sm:justify-between"${_scopeId}><div class="flex items-center gap-3"${_scopeId}>`);
            _push2(ssrRenderComponent(_sfc_main$b, {
              "post-slug": __props.post.slug,
              "likes-count": __props.post.likes_count || 0,
            ...
            if (authUser.value) {
              _push2(ssrRenderComponent(_sfc_main$2, {
                post: __props.post,
                "initial-watchlisted": __props.isWatchlisted
              }, null, _parent2, _scopeId));
            } else {
              _push2(`<!---->`);
            }
            ...
            _push2(ssrRenderComponent(_sfc_main$g, { posts: __props.relatedPosts }, null, _parent2, _scopeId));
            _push2(`</div></article>`);
            ...
            return [
              ...
              createVNode("article", { class: "min-h-screen bg-white" }, [
                ...
                createVNode("div", { class: "max-w-6xl mx-auto px-4 -mt-32 relative z-10" }, [
                  createVNode("div", { class: "bg-white rounded-t-2xl md:rounded-2xl shadow-sm p-6 md:p-10" }, [
                    createVNode("div", { class: "flex flex-wrap gap-2 mb-4" }, [
                      (openBlock(true), createBlock(Fragment, null, renderList(__props.post.categories.slice(0, 3), (category) => {
                      ...
                      __props.post.categories.length > 3 ? (openBlock(), createBlock("span", {
                        key: 0,
                        class: "px-3 py-1 bg-gray-100 text-gray-500 text-sm font-medium rounded-full",
                        title: __props.post.categories.slice(3).map((c) => c.name).join(", ")
                      }, " +" + toDisplayString(__props.post.categories.length - 3), 9, ["title"])) : createCommentVNode("", true)
                    ]),
                    ...
                    __props.post.is_revisit || __props.post.revisits && __props.post.revisits.length > 0 ? (openBlock(), createBlock(_sfc_main$6, {
                      key: 0,
                      "original-post": __props.post.original_post,
                      revisits: __props.post.revisits || [],
                      "is-revisit": __props.post.is_revisit,
                    ...
                    showExcerpt.value ? (openBlock(), createBlock(PullQuote, {
                      key: 1,
                      text: __props.post.excerpt,
                      "author-name": __props.post.user?.name || ""
                    }, null, 8, ["text", "author-name"])) : createCommentVNode("", true),
                    ...
                    ])) : __props.post.format === "dialogue" ? (openBlock(), createBlock(_sfc_main$5, {
                      key: 3,
                      exchanges: __props.post.dialogue_exchanges || [],
                      "primary-author": __props.post.user,
                      "secondary-author": __props.post.secondary_author,
                      intro: __props.post.content
                    }, null, 8, ["exchanges", "primary-author", "secondary-author", "intro"])) : __props.post.format === "visual_essay" ? (openBlock(), createBlock(_sfc_main$4, {
                      key: 4,
                      blocks: __props.post.visual_essay_blocks || []
                    ...
                    createVNode("div", { class: "mx-auto mt-8 flex max-w-[750px] flex-col gap-4 border-t border-gray-200 pt-8 sm:flex-row sm:items-center sm:justify-between" }, [
                      createVNode("div", { class: "flex items-center gap-3" }, [
                        ...
                        authUser.value ? (openBlock(), createBlock(_sfc_main$2, {
                          key: 0,
                          post: __props.post,
                          "initial-watchlisted": __props.isWatchlisted
                        }, null, 8, ["post", "initial-watchlisted"])) : createCommentVNode("", true)
                      ]),
                ...
                createVNode("div", { class: "max-w-7xl mx-auto px-4 py-12" }, [
                  createVNode(_sfc_main$g, { posts: __props.relatedPosts }, null, 8, ["posts"])
                ])
              ]),
