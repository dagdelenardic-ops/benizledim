import { ref, computed, watch, onMounted, onBeforeUnmount, mergeProps, withCtx, unref, openBlock, createBlock, createVNode, createTextVNode, toDisplayString, withModifiers, createCommentVNode, withDirectives, vModelText, Fragment, renderList, vModelCheckbox, useSSRContext } from "vue";
...
import { _ as _sfc_main$1, a as _sfc_main$4 } from "./AdminLayout-CgPpPth9.js";
...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    const hasMeaningfulDraft = computed(() => Boolean(
      form.title?.trim() || form.excerpt?.trim() || plainContent.value || form.scheduled_at || form.tags.length
    ));
    ...
    const saveServerDraft = async () => {
      ...
      try {
        const endpoint = serverDraftId.value ? `/admin/posts/${serverDraftId.value}/autosave` : "/admin/posts/autosave-draft";
        ...
        serverDraftSavedAt.value = data.saved_at || (/* @__PURE__ */ new Date()).toISOString();
    ...
    const submit = (publish = false) => {
      ...
      form.transform((data) => serverDraftId.value ? { ...data, _method: "PUT" } : data).post(endpoint, {
        forceFormData: true,
        onSuccess: () => {
          clearFormDraft();
          localStorage.removeItem(draftKey);
      ...
      _push(ssrRenderComponent(_sfc_main$1, mergeProps({ title: "Yeni Yazı" }, _attrs), {
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            ...
            if (serverDraftStatus.value !== "idle" || serverDraftSavedAt.value) {
              _push2(`<div class="flex items-center justify-between border border-[var(--bi-ink)] bg-[var(--bi-paper)] px-4 py-2 text-xs font-bold text-[var(--bi-muted)]"${_scopeId}>`);
              if (serverDraftStatus.value === "saving") {
                _push2(`<span${_scopeId}>Sunucu taslağı kaydediliyor...</span>`);
              } else if (serverDraftStatus.value === "error") {
                _push2(`<span class="text-red-700"${_scopeId}>Sunucu taslağı kaydedilemedi; yerel taslak korunuyor.</span>`);
              } else {
                _push2(`<span${_scopeId}>Sunucu taslağı kaydedildi: ${ssrInterpolate(formatServerDraftTime(serverDraftSavedAt.value))}</span>`);
              }
              if (serverDraftId.value) {
                _push2(`<span${_scopeId}>#${ssrInterpolate(serverDraftId.value)}</span>`);
              } else {
                _push2(`<!---->`);
              }
              _push2(`</div>`);
            ...
            return [
              createVNode("div", { class: "mx-auto max-w-5xl space-y-6" }, [
                ...
                createVNode("form", {
                  ...
                  serverDraftStatus.value !== "idle" || serverDraftSavedAt.value ? (openBlock(), createBlock("div", {
                    key: 2,
                    ...
                    serverDraftStatus.value === "saving" ? (openBlock(), createBlock("span", { key: 0 }, "Sunucu taslağı kaydediliyor...")) : serverDraftStatus.value === "error" ? (openBlock(), createBlock("span", {
                      key: 1,
                      class: "text-red-700"
                    }, "Sunucu taslağı kaydedilemedi; yerel taslak korunuyor.")) : (openBlock(), createBlock("span", { key: 2 }, "Sunucu taslağı kaydedildi: " + toDisplayString(formatServerDraftTime(serverDraftSavedAt.value)), 1)),
                    serverDraftId.value ? (openBlock(), createBlock("span", { key: 3 }, "#" + toDisplayString(serverDraftId.value), 1)) : createCommentVNode("", true)
                  ])) : createCommentVNode("", true),
                  createVNode("div", null, [
