        ...
        <div class="space-y-6">
            ...
            <div class="bg-white rounded-lg shadow-sm p-6 space-y-4">
                ...
                <form @submit.prevent="submitNew" class="grid md:grid-cols-2 gap-4">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea v-model="newForm.description" rows="3" class="w-full px-3 py-2 border border-gray-300 rounded-lg"></textarea>
                    </div>

                    <div>
                        <label class="block text-sm text-gray-700 mb-1">Etkinlik Tarihi</label>
                        <input v-model="newForm.event_date" type="date" class="w-full px-3 py-2 border border-gray-300 rounded-lg" />
                    </div>

                    <div>
                        <label class="block text-sm text-gray-700 mb-1">Slider Sırası</label>
                        <input v-model="newForm.slider_order" type="number" min="0" class="w-full px-3 py-2 border border-gray-300 rounded-lg" />
                    </div>
                ...
                <table class="w-full">
                    ...
                    <tbody class="divide-y divide-gray-200">
                        ...
                        <tr v-if="events.data.length === 0">
                            <td colspan="4" class="px-6 py-8 text-center text-gray-500">Etkinlik bulunamadı.</td>
                        </tr>
                    </tbody>
                </table>

                <div v-if="events.links" class="px-6 py-4 border-t border-gray-200 flex justify-center">
                    <div class="flex items-center gap-2">
                        <Link
            ...
            <div v-if="editingId" class="bg-white rounded-lg shadow-sm p-6 space-y-4">
                ...
                <form @submit.prevent="submitEdit(events.data.find((item) => item.id === editingId))" class="grid md:grid-cols-2 gap-4">
                    ...
                    <div>
                        <label class="block text-sm text-gray-700 mb-1">Etkinlik Tarihi</label>
                        <input v-model="editForm.event_date" type="date" class="w-full px-3 py-2 border border-gray-300 rounded-lg" />
                    </div>

                    <div>
                        <label class="block text-sm text-gray-700 mb-1">Slider Sırası</label>
                        <input v-model="editForm.slider_order" type="number" min="0" class="w-full px-3 py-2 border border-gray-300 rounded-lg" />
                    </div>
