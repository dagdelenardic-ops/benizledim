...
class RehydrateHiResImages extends Command
{
    ...
    public function handle(): int
    {
        ...
        foreach ($dbPosts as $post) {
            ...
            if (!$jsonPost) { $stats['unmatched']++; $bar->advance(); continue; }
        ...
        if ($dry) $this->warn('DRY RUN — hiçbir dosya yazılmadı.');
        return 0;
    }
    ...
    private function processCover(Post $post, array $jsonPost, bool $dry, bool $force, array &$stats): void
    {
        ...
        if (!$jsonCover) { $stats['cover_skipped']++; return; }
        ...
        if (!$needsUpgrade) { $stats['cover_skipped']++; return; }
        ...
        if ($dry) { $stats['cover_upgraded']++; return; }

        try {
            $resp = Http::timeout(45)->withHeaders(['User-Agent' => 'BenIzledim/1.0'])->get($hiresUrl);
            if (!$resp->successful()) {
    ...
    private function downloadTo(string $url, string $dir, bool $dry, array &$stats, int $postId): ?string
    {
        ...
        if ($dry) { $stats['content_imgs']++; return '/storage/' . $path; }

        try {
            $resp = Http::timeout(45)->withHeaders(['User-Agent' => 'BenIzledim/1.0'])->get($url);
            if (!$resp->successful()) {
    ...
    {
        if ($dry) { $stats['content_imgs']++; return; }
        try {
            $resp = Http::timeout(45)->withHeaders(['User-Agent' => 'BenIzledim/1.0'])->get($url);
            if (!$resp->successful()) {
